...
# cor.test(dff$idade, dff$idade, use = "complete.obs") #%>% tidy() %>% .$p.value %>% pval_string()
```

```{r calculando o valor de p para correlação maior que 0.5}
...
cor.hip(dff$idade, dff$idade)$p.value %>% retorne_p()
```
```{r Teste hipotese para analise numcat}
...
```{r analise numcat com ajuste de fisher-pitman}
library(coin)
