...
group_normality_test <- function(df, col_num, col_cat, type_response = 0) {
  ...
  resultados <- by(df[[col_num]], df[[col_cat]], function(subset) {
    ks.test(subset, "pnorm", mean(subset, na.rm=T), sd(subset, na.rm=T))
  })
...
group_ks_test(df, coluna2, 'grupo', 0)
bartlett.test(df$tempo_ps2, df$grupo)
```
    ...
    if (group_normality_test(df, col_num, col_cat, 1)){
      ...
      if (teste_homogeneidade$p.value > 0.05){
        if (usa_permutacao) {
          ...
          teste_usado = "Fisher-Pitman t-test (coin::independence_test)"
        ...
        if (usa_permutacao) {
          ...
          teste_usado = "Fisher-Pitman Welch's t-test (coin::oneway_test)"
      ...
      if (usa_permutacao) {
        ...
        teste_usado = "Wilcoxon test with permutation (coin::wilcox_test)"
    ...
    if (group_normality_test(df, col_num, col_cat, 1)){
      ...
      if (teste_homogeneidade$p.value > 0.05){
        if (usa_permutacao) {
          ...
          teste_usado = "Fisher-Pitman ANOVA (coin::independence_test)"
        ...
        if (usa_permutacao) {
          ...
          teste_usado = "Welch's ANOVA with permutation (coin::oneway_test)"
          pvalor = coin::oneway_test(df[[col_num]] ~ df[[col_cat]]) %>% coin::pvalue()
        } else {
          # Welch ANOVA padrão
          pvalor = oneway.test(df[[col_num]] ~ df[[col_cat]], var.equal = FALSE)$p.value
      ...
      if (usa_permutacao) {
        ...
        teste_usado = "Kruskal-Wallis with permutation (coin::independence_test)"
        pvalor = coin::independence_test(df[[col_num]] ~ df[[col_cat]]) %>% coin::pvalue()
      } else {
...
n = 100
cat3 = c(rep("A", n), rep("B", n), rep('C', 29))
filtro_A = cat3 == "A"
filtro_B = cat3 == "B"
filtro_C = cat3 == "C"
df = data.frame(cat3 = cat3)
...
df$numerica[filtro_A] = rnorm(n, mean = 10, sd = 20)
df$numerica[filtro_B] = rnorm(n, mean = 30, sd = 40)
#df$numerica[filtro_C] = rnorm(29, mean = 30, sd = 40)
df$numerica[filtro_C] = rpois(29, 40)
#df$numerica[filtro_C] = runif(29)
