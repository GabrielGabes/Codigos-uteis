...
```{r}
rround = function(valor, digitos, flg_notacao_cientifica = T){
  if (abs(valor) < 0.01 && abs(valor) > 0.0009) {formatC(valor, format = "f", digits = 3)} 
    ...
    if (flg_notacao_cientifica == T){
      formatC(valor, format = "e", digits = 4)
    } else {
      if (abs(valor) <= 0.0009){
        '0'
      } else {
...
rround(5115156, 2, flg_notacao_cientifica = F)
rround(0.0000000041212, 2, flg_notacao_cientifica = F)
```


```{r}
# Aplicando a função a todos elementos da coluna 
apply_rround = function(df, coluna, digitos = 4, flg_notacao_cientifica = T){
  df[[coluna]] = sapply(df[[coluna]], function(x) {
    if (!is.na(x) && is.numeric(x)) {
      rround(x, digitos, flg_notacao_cientifica)
    } else {
      x
    }
  })
...
cont = function(df, variavel){
  df %>% tabyl(.data[[variavel]], show_na = FALSE) %>% 
    adorn_pct_formatting(1) %>% as.data.frame()
}
...
conti = function(df, var_y, var_x, sentido_percent='col', apenas_fisher=F){
  ...
  tabela = df %>% 
    ...
    # mutate(across(-1, ~gsub("%", " %", .)))  # melhora o espaçamento
  ...
  indices = c(indices[1], indices[length(indices)-2], indices[2:(length(indices)-3)],
              indices[(length(indices)-1):length(indices)])
...
conti(df, coluna_analisada, coluna, "col")

# conti(dff, "desfecho", "tratamentos")
# conti(df, coluna_analisada, 'asa', sentido_percent='col', apenas_fisher=F)
```
...
normalidade_por_grupo_shapiro = function(df, col_num, col_cat){
  ...
  resultados = tabela$`df[[col_num]]` #`p-value`
  verificacao = any(resultados < 0.05)
...
summary_numerico_parametrico = function(df, col_num){
  tabela = df %>%
    ...
    summarise(
      mean = round(mean(!!sym(col_num), na.rm = TRUE), 1),
      std = round(sd(!!sym(col_num), na.rm = TRUE), 1))
...
summary_numerico_por_grupo_parametrico = function(df, col_num, col_cat, teste_extra="F"){
  ...
  sumario_grupo = df %>%
    ...
    summarise(
      resumo = paste0(
        round(mean(!!sym(col_num), na.rm = TRUE), 1), 
        " ;± ", round(as.numeric(sd(!!sym(col_num), na.rm = TRUE)), 1))
...
summary_numerico_n_parametrico = function(df, col_num){
  tabela = df %>%
    filter(!is.na(!!sym(col_num))) %>%
    summarise(
      median = round(median(!!sym(col_num), na.rm = TRUE), 1),
      q1_q3 = paste0(
        ';[', round(as.numeric(quantile(!!sym(col_num), 0.25, na.rm = TRUE)), 1), 
        " - ", 
        round(as.numeric(quantile(!!sym(col_num), 0.75, na.rm = TRUE)), 1), ']'))
...
summary_numerico_por_grupo_n_parametrico = function(df, col_num, col_cat, teste_extra="F"){
  ...
  sumario_grupo = df %>%
    ...
    summarise(
      resumo = paste0(
        round(median(!!sym(col_num), na.rm = TRUE), 1),
        "; [", round(as.numeric(quantile(!!sym(col_num), 0.25, na.rm = TRUE)), 1), 
        " - ", 
        round(as.numeric(quantile(!!sym(col_num), 0.75, na.rm = TRUE)), 1),"]"
      )
...
summary_numerico_por_grupo_n_parametrico_f = function(df, col_num, col_cat, teste_extra="F"){
  # Sumário por grupo
  sumario_grupo = df %>%
    filter(!is.na(!!sym(col_num)), !is.na(!!sym(col_cat))) %>%
    group_by(!!sym(col_cat)) %>%
    summarise(
      resumo = paste0(
        ...
        " (", round(as.numeric(min(!!sym(col_num), na.rm = TRUE)), 2), 
        "; ", 
        round(as.numeric(max(!!sym(col_num), na.rm = TRUE)), 2),")"
      )
    )
  sumario_grupo = rename(sumario_grupo, "coluna" = col_cat)
  
  sumario_final = sumario_grupo # Combinar os sumários
  tabela = as.data.frame(t(sumario_final)) # Transpor o dataframe
  colnames(tabela) = tabela[1, ] # Ajustar os nomes das colunas
  tabela = tabela[-1, ]  # Remover a primeira linha
  rownames(tabela)[1] = col_num
  ...
  if (length(levels(as.factor(df[[col_cat]]))) > 2){
    ...
    if (teste_extra == "T"){
      ...
      IC_00 = as.character(rround(teste_hip$conf.int[1],2))
      IC_01 = as.character(rround(teste_hip$conf.int[2],2))
      tamnho_do_efeito = paste0(estimador,' (',IC_00,'; ',IC_01,')')
      tabela[["hodges_lehmann"]] = NA
      tabela[["hodges_lehmann"]] = tamnho_do_efeito
      
      # teste_hip = rcompanion::wilcoxonR(df[[col_num]], g = df[[col_cat]], ci = T)
      estimador = as.character(rround(teste_hip$r[1],2))
      IC_00 = as.character(rround(teste_hip$lower.ci[1],2))
      IC_01 = as.character(rround(teste_hip$upper.ci[1],2))
      tamnho_do_efeito = paste0(estimador,' (',IC_00,'; ',IC_01,')')
      tabela[["tmn_efeito01"]] = NA
      tabela[["tmn_efeito01"]] = tamnho_do_efeito
      }
    }
  tabela[["P-value"]] = pvalor
  ...
  rownames(tabela) = NULL
  tabela = tabela[, c(ncol(tabela), 1:(ncol(tabela)-1))] #ultima coluna para primeira
  tabela[["Test_Used"]] = NULL
  
  return(tabela)
}

# summary_numerico_por_grupo_n_parametrico_f(fd, 'ema_9', coluna_analisada, teste_extra = 'T')
```
...
analise_mod = function(modelo, digits_arround = 2, flg_notacao_cientifica = T){
  ...
  odds = apply_rround(odds, "OR", digits_arround, flg_notacao_cientifica)
  odds = apply_rround(odds, "2.5 %", digits_arround, flg_notacao_cientifica)
  odds = apply_rround(odds, "97.5 %", digits_arround, flg_notacao_cientifica)
