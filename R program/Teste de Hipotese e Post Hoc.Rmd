...
library(FSA)
dunnTest(var_num ~ tratamentos, data=df, method="holm")
```


```{r Pós Hoc Bonferroni}
#qualquer teste
...
teste_hip = chisq.test(df$desfecho, df$tratamentos)
teste_hip = chisq.test(tab)
teste_hip

# Valores observados
teste_hip$observed # Oij

# Valores esperados
teste_hip$expected # Eij

# Resíduos
teste_hip$residuals # (Oij - Eij) / sqrt(Eij)

# Resíduos padronizados Ajustados (ASR)
teste_hip$stdres

# p-valor bicaudal por célula (sem correção)
p_raw = 2 * (1 - pnorm(abs(teste_hip$stdres)))
p_raw

# Correção de bonferroni para multiplas comparações
### Transformar matriz em vetor
### Ajuste de p-valor (escolha o método: "holm" ou "bonferroni", por ex.)
p_adj_vec  <- p.adjust(as.vector(p_raw), method = "holm")

# Voltar para formato de matriz
p_adj <- matrix(
  p_adj_vec,
  nrow = nrow(tab),
  ncol = ncol(tab),
...
rownames(p_adj) <- rownames(tab)
colnames(p_adj) <- colnames(tab)
p_adj # p-values ajustados por célula


posthoc_chisq <- function(tab, method = "holm") {
  teste <- chisq.test(tab, correct = FALSE)
  
  stdres <- teste$stdres
  
  p_raw <- 2 * (1 - pnorm(abs(stdres)))
  
  p_adj_vec <- p.adjust(as.vector(p_raw), method = method)
...
# Uso:
res <- posthoc_chisq(tab, method = "holm")
res$teste_global
res$stdres
res$p_ajustado
