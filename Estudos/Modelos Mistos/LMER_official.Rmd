...
source("C:/Users/gabri/OneDrive/Documentos/Codigos úteis/Gerador de Dados Ficticios/dataframe.R", echo=F)
df = dff
```
...
```{r Modelo Linear Simples com intercepto 0}
modelo_lm <- lm(response ~ fixed_effects - 1, data = df)
...
# Grafico de Previsão
ggplot(df, aes(x = fixed_effects, y = response)) +
  geom_point(alpha=0.5, size=2.5) +
  geom_smooth(aes(y = pred), method = "lm", se=TRUE, size = 0.75) +
  labs(title = "Modelo Linear Simples Intecepto 0", x='x', y='y', color='groups')
# ggsave("Modelo Linear Simples Intecepto 0.jpg", height=10, width=15, units="cm", dpi= 600)
```
