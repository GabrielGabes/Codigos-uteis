...
capture = function(tabela, col_names=TRUE){
  tabela %>% print() %>% write_clip(dec = ",", col.names = col_names)
}
...
rround = function(valor, digitos){
  ...
  else if (abs(valor) <= 0.0009 && valor != 0 || abs(valor) > 1000) {"0.00"}#{formatC(valor, format = "e", digits = digitos)} 
  else {formatC(valor, format = "f", digits = digitos)}
}
...
conti_grafi = function(df, coluna_x, coluna_y, sentido_percent='col'){
  ...
  if (sentido_percent == 'row'){ #fazendo o troca a troca kkkkkkkkkkkkkkkkkkkkk #kapa_pride #la_ele
    temp = coluna_x
    coluna_x = coluna_y
    coluna_y = temp
  }
  
  niveis_x = sapply( levels(as.factor(df[[coluna_x]])), function(x) if (!is.na(x)) adicionar_quebra_de_linha(x, 35) else NA)
  niveis_fill = sapply( levels(as.factor(df[[coluna_y]])), function(x) if (!is.na(x)) adicionar_quebra_de_linha(x, 25) else NA)
...
box_vin_jit = function(df, col_num, col_cat){
  ...
  ggplot(df, aes(x=as.factor(!!sym(col_cat)), y=!!sym(col_num), fill=as.factor(!!sym(col_cat)))) + 
    ...
    labs(x=NULL, y=NULL, title=NULL)#, subtitle=subtitulo)
}
